...
%define GET_TAG_REF(r,b) mov r ,qword[b+1]	
...
%macro MALLOC 2
	...
	add rsp, 8
%endmacro
;Can only be called with constant register throws sigfoult
%macro SHIFT_FRAME 1
	push rax
	mov rax, PARAM_COUNT
	add rax, 5
	...
	%rep %1
	dec rax
	mov r12,qword [rbp-WORD_SIZE*i]
	mov  qword[rbp+WORD_SIZE*rax],r12
	%assign i i+1
	%endrep
	pop rax
	shl rbx,3
...
;new macro for nums; %1 is where to save %2 bigin of params of stack before magic %3 is num of prams to make pair
%macro MAKE_OP_PAIR 3 
    mov rsi ,[%2]
    MAKE_PAIR(r9,rsi,SOB_NIL_ADDRESS)
	mov r10 ,r9   
	%%loop22:
		dec %3
		cmp %3 ,0
...
%endmacro
;%1 is where to copy from %2where to copy to %3 distance of copy
%macro COPY_FRAME 3
	%%loop11:
		cmp %3 , 0
		je %%end
	...
	%%end:
%endmacro
;%1 is top %2 is bootom params
%macro REVERSE_PARAMS 3
	%%loop11:
		cmp %3 , 0
		je %%end
		mov r9, [%1]
...
%endmacro
;macro for bool
%macro MAKE_BYTE_LIT 2
	db %1
        db %2
%endmacro
;string macro 2nd arg is len 
%macro MAKE_STRING_LIT 1+
  db T_STRING
  dq (%%end_str - %%str)
  %%str:
    db %1
  %%end_str:
%endmacro
;definitions for Const table
%define MAKE_NIL db T_NIL

%define MAKE_VOID db T_VOID

%define MAKE_BOOL(v) \
        MAKE_BYTE_LIT T_BOOL ,v

%define MAKE_LIT_INT(i) \
        MAKE_WORD_LIT T_INTEGER , i

%define MAKE_LIT_FLOAT(f) \
        MAKE_WORD_LIT T_FLOAT , f

%define MAKE_CHAR_LIT(c) \
        MAKE_BYTE_LIT T_CHAR , c

%define MAKE_SYMBOL_LIT(sym) \
        MAKE_WORD_LIT T_SYMBOL , sym

%define MAKE_TAGREF_LIT(ref)\
		MAKE_WORD_LIT T_REF ,ref

;end of Const tbl definitions
...
write_sob_pair:
	...
	CAR rsi, rsi
	PRINT_REF rsi;NEW print for ref
