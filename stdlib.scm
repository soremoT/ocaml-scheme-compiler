...
(define fold-left (lambda (f base lst)
  (if (null? lst)
      base
      (fold-left f (f base (car lst)) (cdr lst)))))

(define fold-right  (lambda (f base lst)
  (if (null? lst)
      base
      (f (car lst) (fold-right f base (cdr lst)) )
       )))
...
(define cut-last (lambda (lst)
                 (if (null? (cdr lst))
                     '()
                     (cons (car lst) (cut-last (cdr lst)) )
                  )))
(define cons* (lambda  (a . lst)
              (if (null? lst)
                   a
                  (cons a (fold-right cons (get-last lst) (cut-last lst) )))))
